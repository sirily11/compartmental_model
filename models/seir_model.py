...
class SEIRModel(BaseModel):
    ...
    def __init__(self, population=1000000, virus_induced_acerage_fatlity_rate=0.006, per_capita_natural_death_rate=0,
        ...
        super().__init__(labels=['Susceptible', 'Exposed',
                                 'Infectious', 'Recovered'], title='SEIR Model')
    ...
    def s_dot(self):
        ...
        """
        return self.per_capita_birth_rate - self.per_capita_natural_death_rate * self.s - self.probability_of_disease_transmission * self.s * self.i / self.n

    @property
    def e_dot(self):
        return self.probability_of_disease_transmission * self.s * self.i / self.n - (self.per_capita_natural_death_rate + self.rate_of_progression_from_exposed) * self.e

    @property
    def i_dot(self):
        return self.rate_of_progression_from_exposed * self.e - (self.recovery_rate + self.per_capita_natural_death_rate + self.virus_induced_average_fatality_rate) * self.i
